...
public class Main {
    ...
    static int solve(int left, int right, int cnt){
        ...
        dp[left][right][cnt] = Math.min(solve(move[cnt], right, cnt+1) + logic(left, move[cnt]), solve(left, move[cnt], cnt+1)+logic(right, move[cnt]));
        return dp[left][right][cnt];
    }
    static int logic(int pos, int des){
